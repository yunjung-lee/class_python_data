...
def openSQLite() :
    ...
    try :
        # imageDB-imageTable 내의 filename 및 resolution 테이블 목록이 궁금?
        ...
        while True :
            ...
            if row == None:
                break
            tableNameList.append(str(row[0])+":" + str(row[1]));
        ...
        def selectTable() :
            ...
            subWindow.destroy()
            filename, resolution = tableNameList[selectedIndex].split(':')
            sql = "SELECT row, col, value FROM imageTable WHERE filename='" + filename + "' AND resolution=" + resolution
            cur.execute(sql)

            inH = inW = int(resolution)  # 입력메모리 크기 결정! (중요)
            inImage = [];    tmpList = []
            for i in range(inH):  # 입력메모리 확보(0으로 초기화)
                tmpList = []
                for k in range(inW):
            ...
            while True:
                ...
                if row_tuple == None:
                    break
                row = row_tuple[0]; col = row_tuple[1]; value= row_tuple[2]
                inImage[row][col] = value
