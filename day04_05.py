...
def zoomOut():
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH
    # 중요!! 출력 메모리의 크기 결정
    scale = askinteger('축소', '축소할 배수 ==>', minvalue=2, maxvalue=32)
    outW =int(inW/scale)
    outH = int(inH/scale)
    outImage = []
    tmpList = []
    ...
    for i in range(inH):
        for k in range(inW):
            outImage[int(i/scale)][int(k/scale)] = inImage[i][k]  # forword(input->output을 채우는) 방식
    display()

def zoomIn():
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH
    # 중요!! 출력 메모리의 크기 결정
    scale = askinteger('확대', '확대할 배수 ==>', minvalue=2, maxvalue=32)
    outW =inW*scale
    outH = inH*scale
    outImage = []
    tmpList = []
    ...
    for i in range(inH):
        for k in range(inW):
            outImage[i*scale][k*scale] = inImage[i][k]  # forword(input->output을 채우는) 방식
    display()
...
def zoomInRe():
    ...
    for i in range(outW):
        for k in range(outH):
            outImage[int(i)][int(k)] = inImage[int(i/scale)][int(k/scale)]  # forword(input->output을 채우는) 방식
    display()
...
window.title('영상처리 & 데이터분석 Ver 0.3')
...
geoMenu.add_separator()
geoMenu.add_command(label = '전방향 확대',command = zoomIn)
geoMenu.add_separator()
geoMenu.add_command(label = '역방향 확대',command = zoomInRe)
